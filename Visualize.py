...
def show_detections_without_ground_truth(detections, scores, file_names):
    ...
    """
    test_images_path = os.path.join("./validare/validare/", '*.jpg')
    ...
    for test_file in test_files:
        ...
        cv.imwrite(os.path.join("./save_test/", "detections_" + short_file_name), image)
...
def show_detections_with_ground_truth(detections, scores, file_names):
    ...
    ground_truth_bboxes = np.loadtxt("./validare/task1_gt_validare.txt", dtype='str')
    test_images_path = os.path.join("./validare/validare/", '*.jpg')
    test_files = glob.glob(test_images_path)
